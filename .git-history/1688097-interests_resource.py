...
class InterestResource(BaseResource):
    ...
    def _get_cached_response(self, interest, field_set_key):
    ...
    def get(self):
        ...
        if fetch_from_cache:
            cache_key, resp = self._get_cached_response(interest, field_set_key)
    ...
    def _get_interest_for_metadata(self, interest, field_set_key):
        ...
        if fetch_from_cache:
            _, response = self._get_cached_response(interest, field_set_key)
    ...
    def get_page_metadata(self):
        ...
        if response.get('status') == APIStatus.SUCCESS:
            ...
            metadata = metatags.get_interest_metadata(self, interest_data)
...
class BaseInterestsFeedResource(BaseResource):
    ...
    def _get_api_and_cache_bookmarks(self, identifier):
    ...
    def _get_api_klp_feed(self, interest_key, identifier, api_bookmark, page_size):
        debug_data = self._get_debug_data()
        debug_data.update({'page_size': page_size})
        debug_data.update({'bookmark': api_bookmark})
        kwargs = {'data': debug_data}
        kwargs['field_set_key'] = self.get_field_set_key()
        ...
        combined_bookmark = identifier + base64.b64encode(str(0) + ',' + str(api_bookmark))
        self.add_bookmark(combined_bookmark)
        result['data'] = self._repackage_data(result)
        return result

    def _get_base64_number_images(self):
    ...
    def _get_cached_klp_feed(self, interest_id, interest_key, identifier,
                             api_bookmark, client_offset, client_page_size):
        start_time_ms = time_utils.now_millis()
        cache_hit = True
        ...
        if interest_id and client_offset is not None:
            fetch_from_cache, update_cache = self._get_cache_settings()
        else:
            fetch_from_cache = False
            update_cache = False
        ...
        temp_multiple_results = klp_feed_cache.get_many(cache_keys) if fetch_from_cache else None
        ...
        if temp_multiple_results and len(temp_multiple_results) == num_pages_needed:
            ...
            for i in range(num_pages_needed):
                ...
                if result and result.get('data') and result.get('status') == APIStatus.SUCCESS:
                    self._augment_result_data(result)
                    if update_cache and result.get('data')[0].get('id'):
                        cache_page_bookmark = (initial_cache_page_bookmark + (i * cache_page_size))
                        cache_key = (interest_id, self.context.locale.lower(), cache_page_bookmark,
                                     1 if vase_key else 0, field_set_key, experiments)
        ...
        if result.get('data'):
            if (self.context.is_full_page and
                ...
                seo_utils.holdout_leaf_snippet(self, result['data'], ('holdout_klp'))
        ...
        if result.get('status') == APIStatus.SUCCESS:
            opentsdb_client_v2.timing('denzel.resource.InterestsFeedResource.get',
                                      ...
                                      tags=build_metric_tags(self.context, {'cached': str(cache_hit)}))
        base64_pins_count = self._get_base64_number_images()
        if base64_pins_count:
    ...
    def get(self):
        ...
        identifier = 'cb:'
        api_bookmark, client_offset = self._get_api_and_cache_bookmarks(identifier)

        # We return noindex for fake KLPs. We could return empty results for crawlers here to save search capacity.
        if not interest_id and not self.context.is_authenticated and self.context.is_bot == 'true':
            return {'data': {'results': []}}
        elif decider.decide_experiment('unauth_klp_bot_skip_cache') \
                and self.context.is_bot == 'true' and not self.context.is_amp:
            return self._get_api_klp_feed(interest_key, identifier, api_bookmark, page_size)
        else:
            return self._get_cached_klp_feed(interest_id, interest_key, identifier,
                                             api_bookmark, client_offset, page_size)
