...
from data_clients.memcache import create_key_gen_mmh3
...
from settings import DID_IT_EXP_GROUPS
...
klp_feed_cache = MemcacheClient(
    KLP_FEED_CONN,
    create_key_gen_mmh3(KLP_FEED_PREFIX, KLP_FEED_FORMAT),
    KLP_FEED_EXPIRE_TIME_SECONDS)

interest_cache = MemcacheClient(
    KLP_INTEREST_CONN,
    create_key_gen_mmh3(KLP_INTEREST_PREFIX, KLP_INTEREST_FORMAT),
...
class BaseInterestsFeedResource(BaseResource):
    ...
    def get_field_set(self):
        ...
        if field_set_key == 'pins':
            if self.context.experiments.v2_activate_experiment('web_did_it') in DID_IT_EXP_GROUPS:
    ...
    def _get_cached_klp_feed(self, interest_id, interest_key):
        ...
        for seo_experiment in ['unauth_ranking_klp', 'gender_inference']:
        ...
        # For gender inference experiment, only use cache if user is not in experiment.
        gender_inference_group = self.context.experiments.v2_get_group('gender_inference')
        ...
        use_cache = (interest_id and
                     ...
                     gender_inference_group is None)
        ...
        if result:
            ...
            if result and result.get('data') and result.get('status') == APIStatus.SUCCESS:
                ...
                if use_cache and first_pin_id:
                    klp_feed_cache.set(cache_key, zlib.compress(json.dumps(result), 9))
        ...
        if result.get('data'):
            ...
            seo_utils.append_image_sizing_pin_data(self, result['data'], [], True)
