...
class InterestResource(BaseResource):
    ...
    def _in_pure_react_exp(self):
        pure_react_enabled_groups = ['employees', 'enabled', 'enabled1', 'partner_employees', 'partner_enabled']
        return (self.context.experiments.v2_activate_experiment('web_pure_react') in pure_react_enabled_groups)
    ...
    def _auth_klp_refresh_experiment_gating(self):
        ...
        if (self.context.is_authenticated and main_module_name == 'InterestFeedPage'):
            if self.context.is_mobile_agent:
                return self.context.experiments.v2_activate_experiment('mweb_usm_auth_klp_refresh')
            else:
                return self.context.experiments.v2_activate_experiment('web_usm_auth_klp_refresh_v3')
    ...
    def get(self):
        ...
        if not self.context.klp_redirect_exp:
            determine_explore_redirect(data, self.options.get('interest'), self.context)
        ...
        if auth_klp_group == 'enabled_react_klp' or self._in_pure_react_exp():
            ...
            data = {
                'guides_limit': SEARCH_GUIDES_LIMIT,
                'allow_horizontal_guides': True,
                'page_size': AUTH_LP_PAGE_SIZE
            }
            response = self.request('/vx/auth-klp/%s/' % interest, data=data, field_set_key='auth_react_klp')
...
class BaseInterestsFeedResource(BaseResource):
    ...
    field_sets = {
        ...
        'unauth_react_with_board': PinResource.field_sets['unauth_react_grid_item_with_board'],
    }
    ...
    def get_field_set_key(self):
        field_set_key = self.options.get('field_set_key')
        if field_set_key == 'unauth_react':
            board_backlink_umbrella_grp = self.context.activate_seo_or_unauth_experiment('board_backlink_klp_umbrella')
            in_board_backlink_exp = board_backlink_umbrella_grp and board_backlink_umbrella_grp.startswith('enabled')

            if in_board_backlink_exp:
                field_set_key = 'unauth_react_with_board'

        if not field_set_key:
            field_set_key = self.default_field_set_key
...
class ReactKLPResource(BaseResource):
    ...
    def get(self):
        ...
        # If this is the "main" resource, need to fetch interest resource first
        # in order to fill out some parameters ...
        if self.options.get('main_resource'):
            interest_resource = InterestResource(self.context, **self.options)

            interest_response = interest_resource.get()
            if interest_response.get('status', APIStatus.SUCCESS) != APIStatus.SUCCESS:
